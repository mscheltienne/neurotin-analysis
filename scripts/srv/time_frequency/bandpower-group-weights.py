...
for band, (fmin, fmax) in frequencies.items():
    df_fname = BP_FOLDER / f"{band}-onrun-full-group-weights.pcl"
...
for band, (fmin, fmax) in frequencies.items():
    df_fname = BP_FOLDER / f"{band}-onrun-regular-group-weights.pcl"
    df_abs, df_rel = compute_bandpower_onrun(
        DATA_FOLDER,
        DATA_PP_FOLDER,
        valid_only,
...
for band, (fmin, fmax) in frequencies.items():
    df_fname = BP_FOLDER / f"{band}-onrun-transfer-group-weights.pcl"
    df_abs, df_rel = compute_bandpower_onrun(
        DATA_FOLDER,
        DATA_PP_FOLDER,
        valid_only,
